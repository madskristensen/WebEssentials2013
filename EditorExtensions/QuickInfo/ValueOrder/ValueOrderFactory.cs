    ...
    internal static class ValueOrderFactory
    {
        ...
        private static void Margins(ISignatureHelpSession session, IList<ISignature> signatures, Declaration dec, ITrackingSpan span)
        {
            ...
            ValueOrderSignature signature1 = new ValueOrderSignature(
                string.Format("div {{ {0}: {1} {2} {3} {4}; }} ", dec.PropertyName.Text, value1, value2, value3, value4),
                string.Format("[top={0}] [right={1}] [bottom={2}] [left={3}]", value1, value2, value3, value4),
                span, session);

            ValueOrderSignature signature2 = new ValueOrderSignature(
                string.Format("div {{ {0}: {1} {2} {3}; }} ", dec.PropertyName.Text, value1, value2, value3),
                string.Format("[top={0}] [right and left={1}] [bottom={2}]", value1, value2, value3),
                span, session);

            ValueOrderSignature signature3 = new ValueOrderSignature(
                string.Format("div {{ {0}: {1} {2}; }} ", dec.PropertyName.Text, value1, value2),
                string.Format("[top and bottom={0}] [right and left={1}]", value1, value2),
                span, session);

            ValueOrderSignature signature4 = new ValueOrderSignature(
                string.Format("div {{ {0}: {1}; }} ", dec.PropertyName.Text, value1),
                string.Format("[top and right and bottom and left={0}]", value1),
                span, session);
        ...
        private static void Corners(ISignatureHelpSession session, IList<ISignature> signatures, Declaration dec, ITrackingSpan span)
        {
            ...
            ValueOrderSignature signature1 = new ValueOrderSignature(
                string.Format("div {{ {0}: {1} {2} {3} {4}; }} ", dec.PropertyName.Text, value1, value2, value3, value4),
                string.Format("[top-left={0}] [top-right={1}] [bottom-right={2}] [bottom-left={3}]", value1, value2, value3, value4),
                span, session);

            ValueOrderSignature signature2 = new ValueOrderSignature(
                string.Format("div {{ {0}: {1} {2} {3}; }} ", dec.PropertyName.Text, value1, value2, value3),
                string.Format("[top-left={0}] [top-right and bottom-left={1}] [bottom-right={2}]", value1, value2, value3),
                span, session);

            ValueOrderSignature signature3 = new ValueOrderSignature(
                string.Format("div {{ {0}: {1} {2}; }} ", dec.PropertyName.Text, value1, value2),
                string.Format("[top-left and bottom-right={0}] [top-right and bottom-left={1}]", value1, value2),
                span, session);

            ValueOrderSignature signature4 = new ValueOrderSignature(
                string.Format("div {{ {0}: {1}; }} ", dec.PropertyName.Text, value1),
                string.Format("[top-left and top-right and bottom-right and bottom-left={0}]", value1),
                span, session);
